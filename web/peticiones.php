...
// Ejecutamos la SQL devuelve un array con todos los registros.
				...
				if (empty($resultado)) { // Usuario no existe en tabla usuarios.
...
// Empleamos la variable de $_SESSION['usuariofaseregistro'] para insertar sus datos.
		...
		case 14:  // Listado de equipos en formato HTML <TR><TD>....
			if (empty($_POST['filtro']))
				...
				$sql = sprintf("SELECT * FROM %sequipamiento as t1 left join %szonas as t2 on t1.zona=t2.idzona where idmaquina like '%%%s%%' order by t1.idmaquina", BD_PREFIJO_TABLAS, BD_PREFIJO_TABLAS, $mibase->depurarCampo($_POST['filtro']));
		...
		case 16:// Borrado de una máquina en equipamiento.
			$sql = sprintf("delete from %sequipamiento where idmaquina='%s'", BD_PREFIJO_TABLAS, $mibase->depurarCampo($_POST['idmaquina']));
			echo $mibase->ejecutarSQL($sql);
			break;

		case 17: //Edición de máquina.
			$sql = sprintf("update %sequipamiento set idmaquina='%s', ip='%s', descrip='%s',zona='%s' where idmaquina='%s'", BD_PREFIJO_TABLAS, strtolower($mibase->depurarCampo($_POST['idmaquina'])), $mibase->depurarCampo($_POST['ip']), $mibase->depurarCampo($_POST['descrip']), $mibase->depurarCampo($_POST['zona']), $mibase->depurarCampo($_POST['idmaquinaoriginal']));
			echo $mibase->ejecutarSQL($sql);
			break;
		...
		case 19: // Resolución de DNS.
			if (!empty($_POST['idmaquina']))
				echo resolverIP($_POST['idmaquina']);
			else
				echo "";
			break;

		case 20: // Inserción de equipamiento con el generador de máquinas.
			$ip = resolverIP($_POST['idmaquina']);
			$sql = sprintf("insert into %sequipamiento(idmaquina,ip,descrip,zona) values('%s','%s','%s',%s)", BD_PREFIJO_TABLAS, strtolower($mibase->depurarCampo($_POST['idmaquina'])), $mibase->depurarCampo($ip), $mibase->depurarCampo($_POST['descrip']), $mibase->depurarCampo($_POST['zona']));

			if ($mibase->ejecutarSQL($sql) == 'ok') {
				echo 'ok';
			} else {
				echo 'Error insertando: ' . $_POST['idmaquina'];
			}
			break;
	} // fin switch
}
